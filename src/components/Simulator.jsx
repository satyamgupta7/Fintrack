...
export default function Simulator() {
  ...
  const ResultCard = ({ bg, label, value, sub, color }) => (
    <div style={{ background: bg, borderRadius: 8, padding: '10px 12px', flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 10, color: '#94a3b8', marginBottom: 3, whiteSpace: 'nowrap' }}>{label}</div>
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div style={{ fontSize: 12, color: '#64748b' }}>Explore how small changes today create big outcomes tomorrow</div>
      </div>
      ...
      <div className="card section-gap" style={{ borderLeft: '3px solid #0ea5e9', background: '#f0f9ff', marginBottom: 20 }}>
        ...
        {currentSaving > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: '#fff', borderRadius: 8, padding: '8px 12px', border: '1px solid #bae6fd' }}>
            <Lightbulb size={13} color="#0ea5e9" />
            <span style={{ fontSize: 12, color: '#0369a1' }}>
              Your {fmt(currentSaving)}/month saving could grow to <strong>{fmtL(savingsCorpus10)}</strong> in 10 years at {sipRate}% CAGR — try it in the SIP simulator below.
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div className="card" style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, color: '#7c3aed', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10, fontWeight: 700 }}>SIP Growth Simulator</div>

          {/* CAGR presets */}
          <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
            {RATE_PRESETS.map(p => (
              <button key={p.rate} onClick={() => setSipRate(p.rate)} style={{
                padding: '4px 10px', borderRadius: 6, border: '1px solid', cursor: 'pointer', fontSize: 11, fontWeight: 600,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 10 }}>
            <div className="form-group" style={{ margin: 0 }}><label>Monthly SIP</label><input type="number" value={sipAmount} onChange={e => setSipAmount(+e.target.value)} /></div>
            <div className="form-group" style={{ margin: 0 }}><label>Rate % (CAGR)</label><input type="number" value={sipRate} onChange={e => setSipRate(+e.target.value)} /></div>
            <div className="form-group" style={{ margin: 0 }}><label>Years</label><input type="number" value={sipYears} min={1} max={30} onChange={e => setSipYears(+e.target.value)} /></div>
          </div>

          {/* What-if comparison row */}
          <div style={{ background: '#faf5ff', border: '1px solid #e9d5ff', borderRadius: 8, padding: '8px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: '#7c3aed', fontWeight: 600, whiteSpace: 'nowrap' }}>What if ₹</span>
            <input type="number" value={sipAlt} onChange={e => setSipAlt(+e.target.value)}
              style={{ width: 90, background: '#fff', border: '1px solid #e9d5ff', borderRadius: 6, padding: '4px 8px', fontSize: 12, color: '#4c1d95', fontWeight: 700, outline: 'none' }} />
            <span style={{ fontSize: 11, color: '#7c3aed', whiteSpace: 'nowrap' }}>/month instead?</span>
            {altDiff > 0 && <span style={{ fontSize: 11, color: '#16a34a', fontWeight: 700, marginLeft: 'auto', whiteSpace: 'nowrap' }}>+{fmtL(altDiff)} extra</span>}
          </div>
          ...
          <button onClick={() => saveScenario('SIP', 'SIP ' + fmt(sipAmount) + ' × ' + sipYears + 'yr @ ' + sipRate + '% = ' + fmtL(finalCorpus))}
            style={{ marginTop: 10, width: '100%', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 7, padding: '7px 0', fontSize: 12, fontWeight: 600, color: '#7c3aed', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        ...
        <div className="card" style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, color: '#ea580c', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10, fontWeight: 700 }}>Loan Prepayment Simulator</div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
            <div className="form-group" style={{ margin: 0 }}><label>Remaining Balance</label><input type="number" value={loanRemaining} onChange={e => setLoanRemaining(+e.target.value)} /></div>
            <div className="form-group" style={{ margin: 0 }}><label>Current EMI</label><input type="number" value={loanEmi} onChange={e => setLoanEmi(+e.target.value)} /></div>
            <div className="form-group" style={{ margin: 0 }}><label>Interest Rate %</label><input type="number" value={loanRate} onChange={e => setLoanRate(+e.target.value)} /></div>
            <div className="form-group" style={{ margin: 0 }}><label>Extra/Month</label><input type="number" value={extraPayment} onChange={e => setExtraPayment(+e.target.value)} /></div>
          </div>

          {/* 2×2 result grid — no overflow */}
          ...
          <button onClick={() => saveScenario('Loan', 'Loan ' + fmt(loanRemaining) + ' + ' + fmt(extraPayment) + '/mo extra → saves ' + fmt(interestSaved) + ' · ' + monthsSaved + ' months faster')}
            style={{ marginTop: 10, width: '100%', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 7, padding: '7px 0', fontSize: 12, fontWeight: 600, color: '#ea580c', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <Save size={12} /> Save this scenario
          </button>
        </div>
      </div>
      ...
      {/* Savings Rate Impact */}
      <div className="card" style={{ marginBottom: savedScenarios.length ? 20 : 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
          {[
            ...
            <div key={i} style={{ background: s.bg, borderRadius: 10, padding: 16, border: '1px solid ' + s.color + '30' }}>
              ...
              <div style={{ fontSize: 12, color: '#64748b' }}>10 years: ~{fmt(s.saving * 12 * 10 * 1.5)} (with 8% returns)</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {savedScenarios.map(s => (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: s.type === 'SIP' ? '#faf5ff' : '#fff7ed', borderRadius: 8, padding: '8px 12px', border: '1px solid ' + (s.type === 'SIP' ? '#e9d5ff' : '#fed7aa') }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: s.type === 'SIP' ? '#ede9fe' : '#ffedd5', color: s.type === 'SIP' ? '#7c3aed' : '#ea580c' }}>{s.type}</span>
