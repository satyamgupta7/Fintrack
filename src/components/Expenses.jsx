    ...
    <div>
      ...
      {monthsData.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
          {monthsData.map(e => {
            const spend = totalSpend(e), saved = saving(e);
            const spendPct = e.paycheck > 0 ? Math.min(100, Math.round((spend / e.paycheck) * 100)) : 0;
            return (
              <div key={e.id} style={{ background: "var(--bg-card)", borderRadius: 14, border: "1px solid var(--border)", padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)" }}>{MONTHS[e.month - 1]} {e.year}</div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-secondary)", marginTop: 5 }}>{spendPct}% of income spent</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                  {ROWS.filter(r => Number(e[r.key] || 0) > 0).map(r => (
                    <div key={r.key} style={{ background: "var(--bg-muted)", borderRadius: 8, padding: "4px 10px", fontSize: 11, color: "var(--text-secondary)" }}>
      ...
      <AppModal
        ...
        icon={<Receipt size={26} color="var(--gold)" />}
        iconBg="rgba(201,162,39,0.15)"
        title={editId ? "Edit Month" : "Add Month"}
        subtitle={editId ? "Update expense record" : "Add a new monthly record"}
        onSave={handleSave}
      ...
      >
        ...
        <FormField label="Paycheck / Income (₹)">
          <input type="number" value={form.paycheck} onChange={f("paycheck")} placeholder="0" />
        </FormField>
        <div style={{ fontSize: 11, color: "var(--gold)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, marginTop: 4 }}>
          Fixed Expenses
        </div>
        <div className="form-row-2">
          {ROWS.filter(r => r.group === "fixed").map(r => (
            <FormField key={r.key} label={r.label}>
              <input type="number" value={form[r.key]} onChange={f(r.key)} placeholder="0" />
            </FormField>
          ))}
        </div>
        <div style={{ fontSize: 11, color: "#E74C3C", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, marginTop: 4 }}>
          Other Expenses
        </div>
        <div className="form-row-2">
