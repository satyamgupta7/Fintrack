...
function totalSpend(e) {
  return ROWS.reduce((s, r) => s + Number(e[r.key] || 0), 0);
}
function saving(e) {
  return Number(e.paycheck || 0) - totalSpend(e);
}
function avg(arr) {
  return arr.length
...
export default function Expenses() {
  ...
  const monthsData = data.expenses
  ...
  function handleSave() {
    ...
    const item = {
      ...
      year: +form.year,
      month: +form.month,
      paycheck: Number(form.paycheck),
    };
    ROWS.forEach((r) => {
      item[r.key] = Number(item[r.key] || 0);
    });
    if (editId) {
      updateData({
        ...data,
        expenses: data.expenses.map((e) =>
          e.id === editId ? { ...item, id: editId } : e,
        ),
      });
    } else {
      updateData({
        ...data,
        expenses: [...data.expenses, { ...item, id: Date.now() }],
      });
    }
    setShowModal(false);
  }
    ...
    <div>
        ...
        {years.map((y) => (
          <button
            ...
            style={{
              ...
              textDecoration:
                hoveredYear === y && selectedYear !== y ? "underline" : "none",
      ...
      >
        ...
        <div
          className="stat-card"
          style={{
            borderLeft: "3px solid " + (isOverspent ? "#dc2626" : "#16a34a"),
          }}
        >
          ...
          <div
            className="value"
            style={{
              color: isOverspent ? "#dc2626" : "#16a34a",
              fontSize: 22,
              fontWeight: 700,
            }}
          >
          ...
          {isOverspent && (
            ...
            >
              ...
              <span style={{ fontSize: 11, color: "#dc2626", lineHeight: 1.4 }}>
                ⚠️ Spent {fmt(Math.abs(yearTotal.saving))} more than earned in{" "}
                {selectedYear}
              </span>
            </div>
          )}
          ...
          >
            <thead>
              <tr style={{ background: "#f8fafc" }}>
                ...
                <td style={{ ...S.monthHead, background: "#f1f5f9" }}>
                  <div
                    style={{ color: "#64748b", fontWeight: 700, fontSize: 11 }}
                  >
                    AVG
                  </div>
                </td>
            ...
            <tbody>
              <tr style={{ background: "#eff6ff" }}>
                <td
                  style={{ ...S.rowLabel, color: "#0ea5e9", fontWeight: 700 }}
                >
                  PAYCHECK
                </td>
                {monthsData.map((e) => (
                  <td
                    key={e.id}
                    style={{ ...S.cell, color: "#0ea5e9", fontWeight: 700 }}
                  >
                    {fmt(e.paycheck)}
                  </td>
                ))}
              ...
              <tr style={{ background: "#f8fafc" }}>
                <td colSpan={monthsData.length + 2} style={S.sectionHead}>
                  FIXED EXPENSES
                </td>
              </tr>
              {ROWS.filter((r) => r.group === "fixed").map((r, i) => {
                ...
                const pct =
                  avgPaycheck > 0
                    ? Math.round((rowAvg / avgPaycheck) * 100)
                    : 0;
                return (
                  <tr
                    key={r.key}
              ...
              <tr style={{ background: "#fff7ed" }}>
                <td
                  style={{ ...S.rowLabel, color: "#ea580c", fontWeight: 700 }}
                >
                  TOTAL SPEND
                </td>
                {monthsData.map((e) => (
                  <td
                    key={e.id}
                    style={{ ...S.cell, color: "#ea580c", fontWeight: 700 }}
                  >
                    {fmt(totalSpend(e))}
                  </td>
                ))}
              ...
              <tr style={{ background: "#f0fdf4" }}>
                <td
                  style={{ ...S.rowLabel, color: "#16a34a", fontWeight: 700 }}
                >
                  NET SAVING
                </td>
                {monthsData.map((e) => {
                ...
                <td
                  style={{
                    ...S.cell,
                    color: yearTotal.saving >= 0 ? "#16a34a" : "#dc2626",
                    fontWeight: 700,
                    background: yearTotal.saving >= 0 ? "#dcfce7" : "#fee2e2",
                  }}
                >
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          ...
          >
            ...
            <div className="form-row" style={{ gridTemplateColumns: "1fr" }}>
              <div className="form-group">
                ...
                <input
                  ...
                  value={form.paycheck}
                  onChange={(e) =>
                    setForm({ ...form, paycheck: e.target.value })
                  }
                  placeholder="0"
                />
              </div>
            </div>
            <div style={{ ...S.modalSection, color: "#ca8a04" }}>
              FIXED EXPENSES
            </div>
            ...
            <div style={{ ...S.modalSection, color: "#dc2626" }}>
              OTHER EXPENSES
            </div>
            <div className="form-row">
