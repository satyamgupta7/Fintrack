...
function getStyle(s) {
  ...
  if (s.name.toLowerCase().includes("emergency")) {
    const pct =
      Number(s.goal) > 0
        ? Math.round((Number(s.amount) / Number(s.goal)) * 100)
        : 0;
    const color = progressColor(pct);
    return {
      color,
      badge: pct < 30 ? "#fee2e2" : pct >= 100 ? "#dcfce7" : "#fef3c7",
      text: color,
      emoji: "🛡️",
    };
  }
...
function sipContribution(item, fallbackMonthly = 0) {
  const monthly =
    Number(item.monthlyAmount || 0) || Number(fallbackMonthly || 0);
  const instalments =
    monthly > 0
      ? Math.max(1, Math.round(Number(item.amount || 0) / monthly))
      : 0;
...
function health(savingsRate, emiRatio) {
  const s = Number(savingsRate),
    e = Number(emiRatio);
  if (s >= 20 && e <= 35)
    return { label: "Healthy", color: "#16a34a", bg: "#dcfce7", dot: "🟢" };
  if (s >= 10 || e <= 50)
    return { label: "Moderate", color: "#ca8a04", bg: "#fef9c3", dot: "🟡" };
  return {
...
export default function Savings() {
  ...
  const latest = [...data.expenses].sort(
    (a, b) => b.year - a.year || b.month - a.month,
  )[0];
  const monthlyExp = latest
    ? EXPENSE_KEYS.reduce((s, k) => s + Number(latest[k] || 0), 0)
    : 0;
  const monthlyInc = latest ? Number(latest.paycheck || 0) : 0;
  const monthlySave = monthlyInc - monthlyExp;
  const saveRate =
    monthlyInc > 0 ? ((monthlySave / monthlyInc) * 100).toFixed(0) : 0;
  const emiRate =
    monthlyInc > 0
      ? ((Number(latest?.emi || 0) / monthlyInc) * 100).toFixed(0)
      : 0;
  const h = health(saveRate, emiRate);

  const emergency = data.savings.find((s) =>
    s.name.toLowerCase().includes("emergency"),
  );
  const emonths =
    emergency && monthlyExp > 0 ? Number(emergency.amount) / monthlyExp : 0;
  const epct = emergency
    ? Math.min(
        100,
        Math.round(
          (Number(emergency.amount) / Number(emergency.goal || 1)) * 100,
        ),
      )
    : 0;
  ...
  const fallbackSipMonthly =
    sipItems.length === 1 ? Number(latest?.invest || 0) : 0;
  const sipMonthly = sipItems.reduce(
    (s, i) => s + (Number(i.monthlyAmount || 0) || fallbackSipMonthly),
    0,
  );
  ...
  const goalTabs = useMemo(() => {
    const names = data.savings
      .filter(
        (s) => s.type === "Goal" && !s.name.toLowerCase().includes("emergency"),
      )
      .map((s) => s.name);
  ...
  function save() {
    ...
    const item = {
      ...
      monthlyAmount:
        form.type === "SIP" ? Number(form.monthlyAmount || 0) : undefined,
      startDate: form.type === "SIP" ? form.startDate || form.date : undefined,
    };
    updateData({
      ...data,
      savings: editId
        ? data.savings.map((s) => (s.id === editId ? { ...s, ...item } : s))
        : [...data.savings, { ...item, id: Date.now() }],
    });
    setShowModal(false);
  }
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            onClick={() => exportSheetToExcel(data.savings, "Savings")}
          >
      ...
      <div style={{ marginBottom: 20 }}>
        <SummaryCard
          ...
          value={fmt(totalAll)}
          valueColor="#0ea5e9"
          sub={`${fmt(Math.max(0, monthlySave))} this month · ${saveRate}% savings rate ${Number(saveRate) >= 20 ? "✓" : ""}`}
          borderColor="#0ea5e9"
        >
          {monthlyInc > 0 && (
            <div
                ...
                {savingsTrend.map((v, i) => (
                  <span
                    ...
                    style={{
                      ...
                      borderRadius: 3,
                      background:
                        i === savingsTrend.length - 1 ? "#0ea5e9" : "#94a3b8",
                    }}
                  />
                ))}
              </div>
        ...
        {tabList.map((t) => (
          <button
            ...
            style={{
              ...
              borderColor: tab === t ? "#0ea5e9" : "#e2e8f0",
            }}
          >
            {tabEmoji(t)}
            {t}
            <span
              style={{
                ...
                padding: "1px 5px",
                borderRadius: 99,
                background: tab === t ? "rgba(255,255,255,0.25)" : "#f1f5f9",
                color: tab === t ? "#fff" : "#94a3b8",
              }}
            >
              {tabCount(t)}
          ...
          {["amount", "date"].map((col) => (
            <button
              ...
              style={{
                ...
                borderRadius: 7,
                cursor: "pointer",
                background: sortBy === col ? "#f1f5f9" : "#fff",
                color: "#94a3b8",
                fontSize: 11,
                display: "flex",
                alignItems: "center",
      ...
      >
        {filtered.map((s) => {
          ...
          const pct =
            !isSip && goal > 0
              ? Math.min(100, Math.round((amt / goal) * 100))
              : 0;
          const rem = !isSip ? Math.max(0, goal - amt) : 0;
          const mToGoal =
            !isSip && monthlySave > 0 && rem > 0
              ? Math.ceil(rem / (monthlySave * 0.3))
              : null;
          ...
          const contribution = isSip
            ? sipContribution(s, fallbackSipMonthly)
            : null;
          ...
          return (
            ...
            >
              ...
              {!isSip && (
                <>
                  <div
                    style={{ fontSize: 11, color: "#94a3b8", marginBottom: 5 }}
                  >
                    {goal > 0
                      ? fmt(goal) +
                        " target · " +
              ...
              <div style={{ marginTop: 6, fontSize: 11 }}>
                {emgM !== null && (
                  ...
                  >
                    <ShieldCheck size={9} />
                    {emgM.toFixed(1)} / 6 months covered
                  </div>
                )}
                {s.type === "SIP" && contribution?.monthly > 0 && (
                  <>
                    <div style={{ color: st.color }}>
                      {fmt(contribution.monthly)}/month ·{" "}
                      {contribution.instalments} instalments
                    </div>
                    {returns && (
                      <div
                        style={{
                          color: returns.returns >= 0 ? "#16a34a" : "#dc2626",
                          marginTop: 3,
                        }}
                      >
                        Returns: {returns.returns >= 0 ? "+" : "-"}
                        {fmt(Math.abs(returns.returns))} (
                        {returns.pct.toFixed(1)}%)
                      </div>
                    )}
                  </>
                )}
              ...
              <div style={{ display: "flex", gap: 5, marginTop: 10 }}>
                <button
                  onClick={() => {
                    setAction({
                      type: !isSip && pct >= 100 ? "target" : "contribute",
                      item: s,
                    });
                    setActionVal("");
                  }}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          ...
          >
            ...
            {!editId && formKind === "Goal" && (
              ...
              >
                💡 Give it any name — e.g. "Vacation", "New Car", "House Down
                Payment". It will appear as its own tab.
              </div>
            )}
            <div
              style={{
        ...
        <div className="modal-overlay" onClick={() => setAction(null)}>
          ...
          >
            <div className="modal-header">
              ...
              >
                <PlusCircle size={15} color="#0ea5e9" />{" "}
                {actionModal.type === "target"
                  ? "Increase Target"
                  : "Add Contribution"}
              </div>
              <button
            ...
            <div style={{ fontSize: 13, color: "#64748b", marginBottom: 14 }}>
              "{actionModal.item.name}" · current:{" "}
              {fmt(actionModal.item.amount)} · target:{" "}
              {fmt(actionModal.item.goal)}
            </div>
