...
const LOAN_ICONS = {
  Home: Home, Car: Car, "Credit Card": CreditCard,
  Education: GraduationCap, Personal: Banknote, Other: FileText,
};
const LOAN_COLORS = {
...
function loanMonths(remaining, emi, rate) {
  const r = Number(rate) / 100 / 12, rem = Number(remaining), e = Number(emi);
...
export default function Loans() {
  ...
  function handleSave() {
    ...
    const parsed = { ...form, principal: Number(form.principal), remaining: Number(form.remaining), rate: Number(form.rate), emi: Number(form.emi) };
  ...
  const loanIcon = editId
    ? LOAN_ICONS[form.loanType] || FileText
    : LOAN_ICONS[form.loanType] || FileText;
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
          {data.loans.map(loan => {
            const pct = loan.principal > 0
              ? Math.min(100, Math.round(((Number(loan.principal) - Number(loan.remaining)) / Number(loan.principal)) * 100)) : 0;
            ...
            const IconComp = LOAN_ICONS[loanType] || FileText;
            const dueStr = loan.dueDate
              ? new Date(loan.dueDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : null;
            return (
              <div key={loan.id} style={{ background: "var(--bg-card)", borderRadius: 16, border: "1px solid var(--border)", padding: 16 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2 }}>{loanType} · {loan.rate}% p.a.</div>
                    </div>
                  </div>
      ...
      <AppModal
        ...
        icon={React.createElement(loanIcon, { size: 26, color: LOAN_COLORS[form.loanType] || "#95A5A6" })}
        iconBg={`${LOAN_COLORS[form.loanType] || "#95A5A6"}22`}
        title={editId ? "Edit Loan" : "Add Loan"}
        subtitle={editId ? "Update loan details" : "Enter your loan details"}
        onSave={handleSave}
        saveLabel={editId ? "Save" : "Add"}
      >
        <FormField label="Loan Name">
          <input value={form.name} onChange={f("name")} placeholder="e.g. Home Loan" />
        </FormField>
        ...
        <div className="form-row-2">
          <FormField label="Principal (₹)">
            <input type="number" value={form.principal} onChange={f("principal")} placeholder="0" />
          </FormField>
          <FormField label="Remaining (₹)">
            <input type="number" value={form.remaining} onChange={f("remaining")} placeholder="0" />
          </FormField>
        </div>
        <div className="form-row-2">
          <FormField label="Interest Rate (%)">
            <input type="number" value={form.rate} onChange={f("rate")} placeholder="0" />
          </FormField>
          <FormField label="Monthly EMI (₹)">
            <input type="number" value={form.emi} onChange={f("emi")} placeholder="0" />
          </FormField>
        </div>
